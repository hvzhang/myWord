...
//    private final String appKey = "yXmcrX2tlGDfUoNKDwqoRLph";

    // 填写网页上申请的APP SECRET 如 $secretKey="94dc99566550d87f8fa8ece112xxxxx"
    private final String secretKey = "544ca4657ba8002e3dea3ac2f5fdd241";
//    private final String secretKey = "eaa7b1b1e803acf9641597878a2cf4dc";

    // 需要识别的文件
//    private final String filename = "16k_test.pcm";
    private final String filename = "1521533111438.wav";//"16k_test.pcm";

    // 文件格式
//    private final String format = "pcm";
    private final String format = "wav";

    //  1537 表示识别普通话，使用输入法模型。1536表示识别普通话，1736表示英文，使用搜索模型。 其它语种参见文档
//    private final int dev_pid = 1736;
...
//*****************************//
public String run(byte[] audioData) throws IOException, DemoException {
    TokenHolder holder = new TokenHolder(appKey, secretKey, TokenHolder.TTS_SCOPE);
    holder.resfresh();
    String token = holder.getToken();
    String result = null;
    if (methodRaw) {
        result = runRawPostMethod(token, audioData);
    } else {
        result = runJsonPostMethod(token, audioData);
    }
    return result;
}
    private String runRawPostMethod(String token, byte[] audioData) throws IOException, DemoException {
        String url2 = url + "?cuid=" + ConnUtil.urlEncode(cuid) + "&dev_pid=" + dev_pid + "&token=" + token;
        //System.out.println(url2);

        //byte[] content = getFileContent(filename);
        byte[] content = audioData;
    ...
    public String runJsonPostMethod(String token, byte[] audioData) throws DemoException, IOException {

//        byte[] content = getFileContent(filename);
        byte[] content = audioData;//getFileContent(filename);
